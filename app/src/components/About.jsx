export function About() {
  const stats = [
    { value: '250k+', label: 'Global Internships', icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    )},
    { value: '80+', label: 'Countries', icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    )},
    { value: '95%', label: 'Intern Satisfaction', icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    )},
    { value: '1 in 3', label: 'Interns Hired', icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative order-2 lg:order-1">
            <div className="relative">
              {/* Decorative elements */}
              <div className="absolute -top-8 -left-8 w-72 h-72 rounded-full opacity-20" style={{ background: 'linear-gradient(135deg, #00B8A9 0%, transparent 70%)', filter: 'blur(40px)' }}></div>
              <div className="absolute -bottom-8 -right-8 w-72 h-72 rounded-full opacity-20" style={{ background: 'linear-gradient(135deg, #FF8C42 0%, transparent 70%)', filter: 'blur(40px)' }}></div>
              
              {/* Main image */}
              <div className="relative rounded-3xl overflow-hidden" style={{ boxShadow: '0 40px 80px rgba(0, 0, 0, 0.15)' }}>
                <img 
              ...
              <div className="absolute -bottom-10 -right-10 grid grid-cols-2 gap-3">
                {stats.map((stat, index) => (
                  <div key={index} className="p-5 rounded-2xl backdrop-blur-xl" style={{ background: 'rgba(255, 255, 255, 0.95)', boxShadow: '0 20px 40px rgba(0, 0, 0, 0.1)' }}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${index % 2 === 0 ? '#00B8A9' : '#FF8C42'} 0%, ${index % 2 === 0 ? '#1A4D4D' : '#FF6B2B'} 100%)`, color: '#FFFFFF' }}>
                        {stat.icon}
                      </div>
                    </div>
          ...
          <div className="order-1 lg:order-2">
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              <button className="group flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-500 hover:-translate-y-1" style={{ background: 'linear-gradient(135deg, #00B8A9 0%, #1A4D4D 100%)', color: '#FFFFFF', boxShadow: '0 20px 40px rgba(0, 184, 169, 0.3)' }}>
